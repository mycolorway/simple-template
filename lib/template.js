(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define('simple-template', ["jquery",
      "moment-timezone"], function ($, moment) {
    ...
    root.simple['tpl'] = factory(jQuery,
      moment);
  }
...
tpl.filters = {
  strftime: function(val, param) {
    ...
    return time.format(param);
  },
  "default": function(val, param) {
    return val || param;
  },
  readableTime: function(val, param) {
    var date;
    console.log(val);
    if (!((val != null) && (typeof moment !== "undefined" && moment !== null))) {
      return '';
...
<<<<<<< HEAD
    if (simple.readableTime == null) {
      return '';
    }
    param || (param = "YYYY-MM-DD HH:mm:ss Z");
    if (tpl.tz) {
      date = moment.tz(val, param, tpl.tz);
    } else {
      date = moment(val, param);
    }
    return simple.readableTime(date);
=======
    if (moment.fn.readableTime == null) {
      return '';
    }
    param || (param = "ddd MMM DD YYYY HH:mm:ss [GMT]ZZ");
    date = moment.tz(val, param, simple.tz);
    console.log(date.readableTime());
    return date.readableTime();
>>>>>>> fix the bug that readableTime behaves differently with different versions of moment(2.8.3 vs 2.8.4) in spec
  },
  ...
  encodeHtml: function(val, param) {
    return mcw.encodeHtml(val);
  },
  emojify: function(val, param) {
    val = val.replace(/:([a-z0-9\+\-_]+):/g, function(match, flag) {
      if (__indexOf.call(EMOJI_NAMES, flag) >= 0) {
        return "<img alt='" + flag + "' src='/images/emoji/" + flag + ".png' class='emoji' />";
      } else {
        return match;
      }
    });
